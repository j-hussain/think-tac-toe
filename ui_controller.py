...
class Menu(QtWidgets.QMainWindow, menu.Ui_menu):
    ...
    def __init__(self, app):
        ...
        for algorithm in constants.ALGORITHMS.keys():
    ...
    def select_board_size(self, size):
        ...
        self.boardSizeDescription.setText(
            "{size}x{size}: Connect {symbols_needed} symbols in a row to win.".format(**constants.CONFIGURATIONS[size]))
    ...
    def select_algorithm(self):
        ...
        self.algorithmDescription.setText(constants.ALGORITHMS[self.play_algorithm]["description"])
...
class GameDisplay(QtWidgets.QMainWindow, game.Ui_board):
    ...
    def __init__(self, app, menu, human_player, ai_player, algorithm, board_configuration):
        ...
        self.board = Board(constants.CONFIGURATIONS[board_configuration])
        ...
        ai_player.brain = constants.ALGORITHMS[algorithm]["brain"](ai_player, human_player, self.board)
        ai_player.brain.load()
        ai_player.set_mode(constants.SMART)

        # For Q-Learning, ensure epsilon is set to 0 so that we exploit
        if isinstance(ai_player.brain, Brain_QLearning):
            ai_player.brain.epsilon = 0
        ...
        self.boardWidget.setRowCount(self.board.size)
        self.boardWidget.setColumnCount(self.board.size)
        ...
        self.delegate = DrawBorderDelegate(self.board.size)
        ...
        for move in range(self.board.squares):
            ...
            self.boardWidget.setCellWidget(move // self.board.size, move % self.board.size, button)
    ...
    def update_table(self):
        for index, symbol in enumerate(self.board.board.flatten()):
    ...
    def button_clicked(self, move):
        ...
        # Play and evaluate
        game_end, game_winner = self.board.play(self.human_player, move)
    ...
    def play_ai_move(self, board_move, state_move):
        ...
        # Play and evaluate
        game_end, game_winner = self.board.play(self.ai_player, board_move)
    ...
    def evaluate_game(self, game_end, game_winner):
        ...
        if game_end:
            ...
            # Add to winner statS
            self.scores.add_win(game_winner)

            self.update_scores()

            # Update label and set enabled
            if game_winner != constants.DRAW:
    ...
    def update_scores(self):
        self.scoreX.setText(str(self.scores.wins_x))
        self.scoreO.setText(str(self.scores.wins_o))
